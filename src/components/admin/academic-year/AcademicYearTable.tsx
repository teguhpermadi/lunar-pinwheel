...
import { AcademicYear } from '@/lib/api';
...
export default function AcademicYearTable({ academicYears, isLoading, onEdit, onDelete, pagination, onPageChange, onBulkDelete, onSearch }: AcademicYearTableProps) {
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [searchQuery, setSearchQuery] = useState('');

    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedIds(academicYears.map(s => s.id));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const isAllSelected = academicYears.length > 0 && selectedIds.length === academicYears.length;
                        ...
                        <>
                            <thead className="bg-slate-50 dark:bg-slate-800/50">
                                <tr>
                                    ...
                                    <th className="px-4 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Year</th>
                                    <th className="px-4 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Semester</th>
                                    <th className="px-4 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Headmaster</th>
                                ...
                                {academicYears.length === 0 ? (
                                    ...
                                    academicYears.map((year) => (
                                        <motion.tr
                                            key={year.id}
                                            variants={item}
                                            className={`group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors ${selectedIds.includes(year.id) ? 'bg-slate-50 dark:bg-slate-800/30' : ''}`}
                                        >
                                            ...
                                            <td className="px-4 py-5">
                                                <span className="block font-bold text-slate-800 dark:text-slate-200">{year.year}</span>
                                            </td>
                                            <td className="px-4 py-5">
                                                <span className={`px-2 py-1 rounded-md text-xs font-bold ${year.semester === '1' || year.semester === 'Ganjil'
                                                    ? 'bg-blue-50 text-blue-600'
                                                    : 'bg-orange-50 text-orange-600'
                                                    }`}>
                                                    {year.semester === '1' ? 'Ganjil' : year.semester === '2' ? 'Genap' : year.semester}
                                                </span>
                                            </td>
                                            <td className="px-4 py-5">
                                                <div className="flex items-center gap-2">
                                                    {year.user ? (
                                                        <>
                                                            <div className="size-6 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500">
                                                                {year.user.name.charAt(0)}
                                                            </div>
                                                            <span className="text-sm text-slate-600 dark:text-slate-400">{year.user.name}</span>
                                                        </>
                                                    ) : (
                                            ...
                                            <td className="px-4 py-5">
                                                <span className="text-xs text-slate-400">
                                                    {new Date(year.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                                                </span>
                                            </td>
                                            <td className="px-8 py-5 text-right">
